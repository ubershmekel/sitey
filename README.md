...
Add a wildcard DNS A record (`*.yourdomain.com`) pointing to your server's
public IP. This lets Sitey automatically assign subdomains to new services and
issue HTTPS certificates for all of them via Let's Encrypt. Then click **+ Add
domain** in Sitey and enter your hostname. You don't have to use wildcard
domains, but it's useful for getting URLs autogenerated for new services.
...
Go to **Integrations** and click **Create GitHub App automatically** — Sitey
pre-fills all the settings. After creating the app, install it on your personal
account or organization so Sitey can clone your repos and trigger deploys on
push.

### 4. Add your first service

Click **+ Add service** from the dashboard or a domain page:
