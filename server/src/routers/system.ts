...
export const systemRouter = router({
  ...
  listContainers: settledProcedure.query(async () => {
    ...
    const services = await db.service.findMany({
      where: { containerName: { in: containerNames } },
      select: { containerName: true, name: true },
    });
    const serviceNameByContainer = new Map(
      services.map((s) => [s.containerName!, s.name]),
    );
    return containers.map((c) => {
      const n = (c.Names[0] ?? c.Id.slice(0, 12)).replace(/^\//, "");
      const serviceName = serviceNameByContainer.get(n) ?? null;
      return {
        id: c.Id.slice(0, 12),
        fullId: c.Id,
        name: n,
