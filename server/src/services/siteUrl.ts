...
async function getConfiguredPublicSiteUrl(): Promise<string | null> {
  const row = await db.systemConfig.findUnique({ where: { key: SITEY_PUBLIC_URL_KEY } })
...
export async function resolvePublicSiteUrl(): Promise<PublicSiteUrlResolution> {
  ...
  if (isUsablePublicUrl(configuredUrl)) {
    return { configuredUrl, wildcardUrl, envUrl, effectiveUrl: configuredUrl, source: 'config' }
  }
  if (isUsablePublicUrl(wildcardUrl)) {
    return { configuredUrl, wildcardUrl, envUrl, effectiveUrl: wildcardUrl, source: 'wildcard' }
  }
  if (isUsablePublicUrl(envUrl)) {
    return { configuredUrl, wildcardUrl, envUrl, effectiveUrl: envUrl, source: 'env' }
  }
  return { configuredUrl, wildcardUrl, envUrl, effectiveUrl: null, source: 'none' }
}
